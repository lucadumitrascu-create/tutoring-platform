...
export default function NewGroupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (err || !group) {
        setError(err?.message || 'Nu s-a putut crea grupul.');
        setSaving(false);
        return;
      }
  ...
  return (
    <div className="max-w-xl">
      <Link href="/admin/groups" className="text-sm text-sketch-dark hover:underline mb-6 inline-block">&larr; Înapoi la grupuri</Link>
      <h1 className="text-2xl font-bold font-hand text-ink mb-1">Creează grup nou</h1>
      <p className="text-ink-lighter mb-8">Configurează o nouă clasă pentru elevii tăi.</p>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-ink mb-1.5">Nume grup *</label>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-2">
          <button type="submit" disabled={saving}
            ...
            {saving ? 'Se creează...' : 'Creează grup'}
          </button>
          <Link href="/admin/groups" className="text-sm text-ink-lighter hover:text-ink font-medium px-4 py-3 min-h-[44px] flex items-center justify-center">Anulează</Link>
        </div>
      </form>
    </div>
  );
